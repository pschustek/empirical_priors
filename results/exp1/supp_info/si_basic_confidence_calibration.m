...
for s=subInd
    ...
    for j=1:size(mask,2)
        ...
        sub(s).conf(j) = mean(C(mask(:,j)),'omitnan');
        sub(s).std(j) = std(C(mask(:,j)),'omitnan');
    end
    ...
    for j=1:size(mask,2)
        ...
        opt(s).conf(j) = mean(optC(mask(:,j)),'omitnan');
    end
end
...
h(2) = errorbar(binCenter,allMean,allSEM,'Color','k','LineWidth',LW,'LineStyle','-','CapSize',3);
...
%% Print
print(gcf, '-dpng', '-r400', '.\..\..\..\plots\exp1\supp_info\si_basic_confidence_calibration.png');
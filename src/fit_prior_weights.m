function [ fweights, R2 ] = fit_prior_weights( Xdat, Ydat, numWeights )
...
R2 = nan(numel(num),1);
fweights = nan(numel(num),max(num));
k = 0;
for j=unique(numWeights)'
    k = k + 1;
    X = Xdat(numWeights==j,1:j);
    Y = Ydat(numWeights==j);

    f_min = @(w) -f_obj( prior_trialWeights(w,X),Y);
    [w,~,~] = fmincon(f_min,ones(1,j)*S,[],[],[],[],ones(1,j)*L,ones(1,j)*U,[],opts); 

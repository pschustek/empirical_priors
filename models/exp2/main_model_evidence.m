...
for s=subInd
    ...
    for m=1:length(fnames)    
        ...
        if ~isempty(getfield(cvll_prior(s),fnames{m})) 
            
            splits = cvll_prior(s).(fnames{m}).splits;
            nTrain = sum(splits(:,1),1);
            nTest = size(splits(:,1),1) - nTrain;
            
            % Get model evidence for given model
            cv_dist = getfield(getfield(cvll_prior(s),fnames{m}),'LLH');
            ...
            % R^2
            tmp = getfield(getfield(cvll_prior(s),fnames{m}),'Rsq');
